 ...
 */
public class MethodScraperForUnaryFunctions {

  private static MethodScraperForUnaryFunctions s_instance;

  public MethodScraperForUnaryFunctions() {
  }

  public static MethodScraperForUnaryFunctions getInstance() {
    return s_instance;
  }
   ...
   */
  public static UnaryFunction<OGArray<? extends Number>, OGArray<? extends Number>>[] availableMethodsForUnaryFunctions(
      Map<Class<?>, Set<UnaryFunction<OGArray<? extends Number>, OGArray<? extends Number>>>> functionSet, Class<?> anInterface) {
    @SuppressWarnings("unchecked")
    UnaryFunction<OGArray<? extends Number>, OGArray<? extends Number>>[] ret = new UnaryFunction[10];
    // reflect on methods
    // lookup arg types
    // check arg types in map to ints
    // dump infix op code into array in right position
    Set<UnaryFunction<OGArray<? extends Number>, OGArray<? extends Number>>> functions = functionSet.get(anInterface);
    if (functions != null) { // only run if there are actually some implementation of the inteface available
      Method[] m;
      Iterator<UnaryFunction<OGArray<? extends Number>, OGArray<? extends Number>>> i = functions.iterator();
      while (i.hasNext()) {
        UnaryFunction<OGArray<? extends Number>, OGArray<? extends Number>> next = i.next();
   ...
   */
  @SuppressWarnings("unchecked")
  public static UnaryFunctionChain[] computeFunctions(OGMatrix conversionTableEvalCost, UnaryFunction<OGArray<? extends Number>, OGArray<? extends Number>>[] functionTable,
    ...
    for (int i = 0; i < n; i++) {
      if (availabilityAndCostMatrix[i] != 0) {
        UnaryFunction<OGArray<? extends Number>, OGArray<? extends Number>> clazz = functionTable[i];
        ret[i] = new UnaryFunctionChain(clazz);
      } else {
        best = -1;

...
public class MethodScraperForInfixOperators {
  ...
  private static Logger _log = LoggerFactory.getLogger(MethodScraperForInfixOperators.class);
  ...
  public static InfixOperator<OGArray<? extends Number>, OGArray<? extends Number>, OGArray<? extends Number>>[][] availableMethodsForInfixOp(
    ...
    if (functions != null) { // only run if there are actually some implementation of the interface available
      ...
      while (i.hasNext()) {
        ...
        if (verbose) {
          _log.info("Analysing class... " + current.getSimpleName());
        }
      ...
      for (int j = 0; j < n; j++) {
        ...
        if (availabilityAndCostMatrix[i][j] != 0) { // there is a method present for dealing with this combination
          ...
          while (it.hasNext()) { // walk list of
            ...
            //            System.out.println("Trying to get from " + MatrixTypeToIndexMap.getClassFromIndex(i).getSimpleName() +
            //            " to " + MatrixTypeToIndexMap.getClassFromIndex(typeArg1).getSimpleName());
            //            System.out.println("Trying to get from " + MatrixTypeToIndexMap.getClassFromIndex(j).getSimpleName() +
            //            " to " + MatrixTypeToIndexMap.getClassFromIndex(typeArg2).getSimpleName());

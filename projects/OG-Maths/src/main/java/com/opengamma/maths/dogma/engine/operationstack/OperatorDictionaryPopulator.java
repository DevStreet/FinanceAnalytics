 ...
 */
public class OperatorDictionaryPopulator<T> {

  public OperatorDictionaryPopulator() {
    populate();
  }
  private final Map<Class<?>, Set<T>> _opMap = new TreeMap<Class<?>, Set<T>>(new ClassComparer());
  ...
  public void populate() {
    if (!_haveScanned.getAndSet(true)) {
      Set<String> classNamesWithAnnotation = ClasspathUtilities.getClassNamesWithAnnotation(DOGMAMethodHook.class);

...
public class DBINOM {
  ...
  static {
    s_dBILNMX = Math.log(D1MACH.two() - (double) 0.0001e0);
    s_dFINTMX = (double) 0.9e0 / D1MACH.three();
    s_log = LoggerFactory.getLogger(DBINOM.class);
  }

  public static double dbinom(int n, int m) {
    ...
    if (k < 9) {
      s_log.warn("Result overflows because N AND/OR M is too big, precision lost.");
    }
    xn = n + 1;
    xk = k + 1;
    xnk = n - k + 1;

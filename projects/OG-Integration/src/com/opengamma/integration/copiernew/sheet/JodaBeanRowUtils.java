 ...
 */
public class JodaBeanRowUtils {

  private static final Logger s_logger = LoggerFactory.getLogger(JodaBeanRowUtils.class);

  private Class<?> _clazz;
  private String[] _ignoredMetaProperties;
  private String[] _classPackages;
  private Map<Class<?>, Class<?>[]> _subClasses;

  protected SortedMap<String, Class<?>> _columns;

  public JodaBeanRowUtils(Class<? extends DirectBean> clazz, String[] ignoredMetaProperties, String[] classPackages, Map<Class<?>, Class<?>[]> subClasses) {
    ArgumentChecker.notNull(clazz, "clazz");
    ArgumentChecker.notNull(ignoredMetaProperties, "ignoredMetaProperties");
    ArgumentChecker.notNull(classPackages, "classPackages");
    ArgumentChecker.notNull(subClasses, "subClasses");

    _clazz = clazz;
    _ignoredMetaProperties = ignoredMetaProperties;
    _classPackages = classPackages;
    _subClasses = subClasses;

    try {
      clazz.getMethod("meta", null).invoke(null);
    } catch (Throwable e) {
    ...
    _columns = constructColumnMap(clazz, "");
  }
  ...
  protected SortedMap<String, Class<?>> constructColumnMap(Class<?> clazz, String prefix) {
    ...
    for (MetaProperty<?> metaProperty : JodaBeanUtils.metaBean(clazz).metaPropertyIterable()) {
      ...
      if (!ignoreMetaProperty(metaProperty)) {
        ...
        if (isBean(metaProperty.propertyType()) && !isConvertible(metaProperty.propertyType())) {
          ...
          columns.putAll(constructColumnMap(beanClass, prefix + metaProperty.name() + ":"));

          // Identify ALL subclasses of this bean and extract all their properties
          for (Class<?> subClass : _subClasses.get(beanClass)) {
            columns.putAll(constructColumnMap((Class<DirectBean>) subClass, prefix + metaProperty.name() + ":"));
          }
        }
      }
    }
  ...
  public DirectBean constructBean(Map<String, String> row) {
    return constructBean(row, _clazz, "");
  }

  public DirectBean constructBean(Map<String, String> row, Class<?> clazz, String prefix) {
    try {
        ...
        if (!ignoreMetaProperty(metaProperty)) {
          ...
          if (isBean(metaProperty.propertyType()) && !isConvertible(metaProperty.propertyType())) {
            ...
            builder.set(metaProperty.name(),
                constructBean(row, beanClass, prefix + metaProperty.name() + ":"));
  ...
  public Map<String, String> constructRow(DirectBean bean) {
    return constructRow(bean, "");
  }

  public Map<String, String> constructRow(DirectBean bean, String prefix) {
    ...
    for (MetaProperty<?> metaProperty : bean.metaBean().metaPropertyIterable()) {
      ...
      if (!ignoreMetaProperty(metaProperty)) {
        ...
        if (isBean(metaProperty.propertyType()) && !isConvertible(metaProperty.propertyType())) {
          ...
          result.putAll(constructRow((DirectBean) metaProperty.get(bean), prefix + metaProperty.name() + ":"));
   ...
   */
  protected static String listToString(List<?> i) {
    String result = "";
    for (Object o : i) {
   ...
   */
  protected static List<?> stringToList(String raw, Class<?> t) {
  ...
  protected Class<DirectBean> getClass(String className) {
    ...
    for (String prefix : _classPackages) {
      try {
  ...
  private static boolean isBean(Class<?> clazz) {
    return DirectBean.class.isAssignableFrom(clazz);
  }
  ...
  private boolean ignoreMetaProperty(MetaProperty<?> mp) {
    ...
    for (String t : _ignoredMetaProperties) {
